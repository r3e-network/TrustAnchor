...
export function useTrustAnchor(): UseTrustAnchorReturn {
  ...
  // Fetch agents list (defined before fetchContractState to avoid forward reference)
  ...
  const fetchContractState = useCallback(async () => {
    ...
    try {
      ...
      // Fetch total stake and RPS, then batch-update stakeInfo once
      const totalStakeResult = await invokeRead(contractHash, "totalStake", []);
      const rpsResult = await invokeRead(contractHash, "rps", []);

      const stakeUpdate: Partial<StakeInfo> = {};
      if (totalStakeResult.length > 0 && totalStakeResult[0].value !== undefined) {
        stakeUpdate.totalStake = formatNeo(String(totalStakeResult[0].value));
      }
      if (rpsResult.length > 0 && rpsResult[0].value !== undefined) {
        stakeUpdate.rps = String(rpsResult[0].value);
      }
      if (Object.keys(stakeUpdate).length > 0) {
        setStakeInfo((prev) => ({ ...prev, ...stakeUpdate }));
      }
  ...
  }, [contractHash, walletHook.rpcClient, invokeRead, fetchAgents]);

  // Fetch user stake info
  const fetchUserStakeInfo = useCallback(
    async (userAddress: string) => {
      if (!contractHash || !walletHook.rpcClient || !userAddress) return;

      try {
        // Fetch stake and reward, then batch-update stakeInfo once
        const stakeResult = await invokeRead(contractHash, "stakeOf", [userAddress]);
        const rewardResult = await invokeRead(contractHash, "rewardOf", [userAddress]);

        const update: Partial<StakeInfo> = {};
        if (stakeResult.length > 0 && stakeResult[0].value !== undefined) {
          update.stake = formatNeo(String(stakeResult[0].value));
        }
        if (rewardResult.length > 0 && rewardResult[0].value !== undefined) {
          update.reward = fromFixed8(String(rewardResult[0].value));
        }
        if (Object.keys(update).length > 0) {
          setStakeInfo((prev) => ({ ...prev, ...update }));
        }
      } catch (err) {
  ...
  const withdraw = useCallback(
    async (amount: number): Promise<TransactionResult> => {
      ...
      try {
        ...
        return invoke(contractHash, "withdraw", [u.HexString.fromHex(scriptHash), amount]);
      } catch (err) {
        return {
          txid: "",
          status: "error",
  ...
  const claimReward = useCallback(async (): Promise<TransactionResult> => {
    ...
    try {
      const scriptHash = wallet.getScriptHashFromAddress(address);
      return invoke(contractHash, "claimReward", [u.HexString.fromHex(scriptHash)]);
    } catch (err) {
      return {
        txid: "",
        status: "error",
  ...
  const emergencyWithdraw = useCallback(async (): Promise<TransactionResult> => {
    ...
    try {
      const scriptHash = wallet.getScriptHashFromAddress(address);
      return invoke(contractHash, "emergencyWithdraw", [u.HexString.fromHex(scriptHash)]);
    } catch (err) {
      return {
        txid: "",
        status: "error",
  ...
  const registerAgent = useCallback(
    async (agentHash: string, target: string, name: string): Promise<TransactionResult> => {
      if (!contractHash) {
        return { txid: "", status: "error", message: "Contract not initialized" };
      }
      if (!isOwner) {
  ...
  const updateAgentTarget = useCallback(
    async (index: number, target: string): Promise<TransactionResult> => {
      if (!contractHash) {
        return { txid: "", status: "error", message: "Contract not initialized" };
      }
      if (!isOwner) {
  ...
  const updateAgentName = useCallback(
    async (index: number, name: string): Promise<TransactionResult> => {
      if (!contractHash) {
        return { txid: "", status: "error", message: "Contract not initialized" };
      }
      if (!isOwner) {
  ...
  const setAgentVoting = useCallback(
    async (index: number, amount: number): Promise<TransactionResult> => {
      if (!contractHash) {
        return { txid: "", status: "error", message: "Contract not initialized" };
      }
      if (!isOwner) {
  ...
  const proposeOwner = useCallback(
    async (newOwner: string): Promise<TransactionResult> => {
      ...
      return invoke(contractHash, "proposeOwner", [newOwner]);
    },
    [contractHash, isOwner, invoke],
  );

  const acceptOwner = useCallback(async (): Promise<TransactionResult> => {
    if (!contractHash) {
      return { txid: "", status: "error", message: "Contract not initialized" };
    }

    return invoke(contractHash, "acceptOwner", []);
  }, [contractHash, invoke]);

  const cancelOwnerProposal = useCallback(async (): Promise<TransactionResult> => {
    if (!contractHash) {
      return { txid: "", status: "error", message: "Contract not initialized" };
    }
    if (!isOwner) {
